...
from vortex_runtime.average_oracle_amplifier_frontier import (
    ...
    average_coordinate_accuracy_from_exact_row_fraction,
    derive_audit,
    fourier_list_size_upper_bound,
    minimum_self_contained_state_bits,
    minimum_exact_row_fraction_for_average_advantage,
    minimum_rank_covering_row_forms,
...
class AverageOracleAmplifierFrontierTests(unittest.TestCase):
    ...
    def test_concentrated_rows_defeat_common_advantage_inference(self) -> None:
        fraction = Fraction(HOT_BITS, REGISTERED_BINARY_BITS)
        accuracy = average_coordinate_accuracy_from_exact_row_fraction(
            fraction,
            field_order=2,
        )
        self.assertEqual(accuracy - Fraction(1, 2), fraction / 2)
        self.assertGreater(accuracy, Fraction(58, 100))

    def test_direct_row_lottery_needs_one_full_rank_basis(self) -> None:
        forms, coefficients = minimum_rank_covering_row_forms(
            output_dimension=16_384,
            input_dimension=16_384,
        )
        self.assertEqual(forms, 16_384)
        self.assertEqual(coefficients, 16_384**2)

    def test_average_advantage_to_exact_row_fraction_is_exact(self) -> None:
        self.assertEqual(
            minimum_exact_row_fraction_for_average_advantage(
                Fraction(1, 100),
                field_order=2,
    ...
    def test_registered_common_row_advantage_is_astronomically_small(self) -> None:
    ...
    def test_audit_preserves_amplifier_and_general_probe_boundary(self) -> None:
        ...
        self.assertFalse(
            audit["claim_boundary"]["published_average_distance_oracle_rejected"]
        )
        self.assertTrue(
            audit["published_premise_correction"][
                "counterexample_satisfies_accuracy_part_only"
            ]
        )
        self.assertFalse(
            audit["published_premise_correction"][
                "counterexample_has_near_linear_query_time"
            ]
        )
        self.assertFalse(
