...
def test_strassen_exact_rank_reduces_power_of_two_cube() -> None:
    ...
    oracle = MultiplicationOracle((strassen,), maximum_depth=6, structured_enabled=False)
...
def test_composition_cannot_be_worse_than_either_arm() -> None:
    strassen = UniformScheme(2, 2, 2, 7, "strassen")
    plain = MultiplicationOracle((strassen,), maximum_depth=5, structured_enabled=False)
    structured = MultiplicationOracle((), maximum_depth=5, structured_enabled=True)
    combined = MultiplicationOracle((strassen,), maximum_depth=5, structured_enabled=True)
    shape = (96, 216, 144)
...
def test_catalog_orientation_and_safe_dominance() -> None:
    rows = [
        {"status": "eligible", "shape": [2, 2, 2], "rank": 7, "key": "s"},
        {"status": "eligible", "shape": [2, 2, 2], "rank": 8, "key": "bad"},
        {"status": "eligible", "shape": [2, 2, 3], "rank": 11, "key": "r"},
        {"status": "exact_but_out_of_scope", "shape": [9, 9, 9], "rank": 1, "key": "skip"},
    ]
    schemes = catalog_schemes(rows)
    assert all(row.rank < row.classical_rank for row in schemes)
    assert any((row.a, row.b, row.c, row.rank) == (2, 2, 2, 7) for row in schemes)
    assert not any(row.source == "bad" for row in schemes)


def test_weighted_ratio_counts_repetition() -> None:
    oracle = MultiplicationOracle((), maximum_depth=0, structured_enabled=False)
