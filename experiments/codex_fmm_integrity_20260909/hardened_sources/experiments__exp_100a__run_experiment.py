...
def validate_config(config: Mapping[str, Any]) -> None:
    ...
    for cap in ("maximum_orientations", "beam_width", "per_split_survivors",
                "maximum_structural_plans_per_shape", "maximum_oracle_plans_per_shape",
                "maximum_family_candidates", "joint_io_bins"):
        if int(search[cap]) <= 0:
            raise RuntimeError(f"search cap must be positive: {cap}")
...
def search_shape_block(
    *,
    ...
    evaluated_count = rejected_count = admissible_count = oracle_evaluated_count = 0
    minimum_workspace: int | None = None
    minimum_workspace_witness: dict[str, Any] | None = None
    workspace_limit = int(config["target_contract"]["peak_vram_bytes"])
        ...
        for state in states:
            ...
            for cut in range(depth + 1):
                ...
                for static_bytes in widths:
                    ...
                    evaluated_count += 1
                    if plan.favorable_workspace_bytes > workspace_limit:
                        rejected_count += 1
                    else:
                        admissible_count += 1
                    if minimum_workspace is None or plan.favorable_workspace_bytes < minimum_workspace:
                        minimum_workspace = plan.favorable_workspace_bytes
                        minimum_workspace_witness = {
                            "scheme_ids": list(state.scheme_ids),
                            "cut_depth": cut,
                            "static_scalar_bytes": static_bytes,
                            "favorable_workspace_bytes": minimum_workspace,
                        }
        direct_plans = list(
    ...
    return SearchResult(
        ...
        accounting={
            "evaluated_direct_plan_count": evaluated_count,
            "evaluated_oracle_plan_count": oracle_evaluated_count,
            "workspace_rejected_count": rejected_count,
            "workspace_admissible_count": admissible_count,
            "workspace_limit_bytes": workspace_limit,
            "minimum_generated_workspace_bytes": minimum_workspace,
            "minimum_workspace_witness": minimum_workspace_witness,
            "completed_evaluated_depths": completed_depths,
            "termination": "beam_exhausted" if state_counts and state_counts[-1] == 0 else "maximum_depth_reached",
            "retained_search_completed": True,
            "scope": "generated retained-beam plans only; no exhaustive catalog or global architecture bound",
        },
    )
...
def evaluate_blocks(
    *,
    ...
    for block_length in [int(value) for value in search["block_lengths"]]:
        ...
        def selection_status(plan: JointPlan | None, candidates: Mapping[str, Sequence[Any]]) -> dict[str, Any]:
            missing = sorted(name for name, options in candidates.items() if not options)
            return {
                "status": ("SELECTED" if plan is not None else
                           "MISSING_FAMILY_CANDIDATE" if missing else
                           "NO_PLAN_IN_FROZEN_IO_BIN_DP"),
                "missing_families": missing,
                "scope": "retained family candidates and frozen conservative I/O bins only",
            }
        row = {
            "block_length": block_length,
            ...
            "direct_selection": selection_status(direct, direct_candidates),
            "oracle_selection": selection_status(oracle, oracle_candidates),
        }
...
def search_integrity_failures(
    searches: Mapping[Any, SearchResult], *, search_contract: Mapping[str, Any],
    expected_keys: set[Any] | None = None
) -> list[str]:
    failures: list[str] = []
    if not searches:
        failures.append("empty_shape_search_collection")
    if expected_keys is not None and set(searches) != expected_keys:
        failures.append("missing_or_unregistered_shape_search")
    for result in searches.values():
        audit = result.accounting or {}
        counts = result.state_count_by_depth
        first = int(search_contract["minimum_depth"])
        last = int(search_contract["maximum_depth"])
        width_count = len(search_contract["static_scalar_bytes"])
        expected_depths = [d for d, count in enumerate(counts, 1) if count > 0 and d >= first]
        expected_direct = sum(counts[d - 1] * (1 + d * width_count) for d in expected_depths)
        expected_oracle = sum(counts[d - 1] for d in expected_depths)
        valid_counts = bool(counts) and len(counts) <= last and all(count > 0 for count in counts[:-1])
        terminal = ("beam_exhausted" if counts and counts[-1] == 0 else
                    "maximum_depth_reached" if counts and counts[-1] > 0 and len(counts) == last else None)
        generated = audit.get("evaluated_direct_plan_count", 0)
        rejected = audit.get("workspace_rejected_count", -1)
        admissible = audit.get("workspace_admissible_count", -1)
        minimum = audit.get("minimum_generated_workspace_bytes")
        limit = audit.get("workspace_limit_bytes")
        consistent = (
            audit.get("retained_search_completed") is True
            and valid_counts and terminal is not None
            and audit.get("termination") == terminal
            and audit.get("completed_evaluated_depths") == expected_depths
            and generated == expected_direct
            and audit.get("evaluated_oracle_plan_count") == expected_oracle
            and generated > 0 and rejected >= 0 and admissible >= 0
            and generated == rejected + admissible
            and isinstance(minimum, int) and isinstance(limit, int)
            and bool(result.direct_structural_plans) == (admissible > 0)
            and ((minimum <= limit) == (admissible > 0))
            and bool(audit.get("completed_evaluated_depths"))
        )
        if not consistent:
            failures.append("incomplete_or_inconsistent_shape_search_accounting")
    if any(not result.oracle_structural_plans for result in searches.values()):
        failures.append("empty_oracle_shape_search")
...
def execute(config_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    expected_keys = {
        (int(block), family.rows, family.columns)
        for block in config["search"]["block_lengths"] for family in families
    }
    integrity_failures.extend(search_integrity_failures(
        searches, search_contract=config["search"], expected_keys=expected_keys))
    ...
    deterministic_core = {
        ...
        "authoritative_decision_scope": "only programs retained/evaluated by the frozen catalog, orientation, beam, cut, family and I/O-bin search; no universal FMM or native impossibility",
    }
