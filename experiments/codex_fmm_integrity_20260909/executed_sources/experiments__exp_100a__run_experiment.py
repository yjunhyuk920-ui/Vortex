...
def search_shape_block(
    *,
    ...
    evaluated_count = rejected_count = admissible_count = 0
...
def search_integrity_failures(
    searches: Mapping[Any, SearchResult], *, expected_keys: set[Any] | None = None
) -> list[str]:
    failures: list[str] = []
    if not searches:
...
def execute(config_path: Path, output_dir: Path) -> dict[str, Any]:
    ...
    integrity_failures.extend(search_integrity_failures(searches, expected_keys=expected_keys))
