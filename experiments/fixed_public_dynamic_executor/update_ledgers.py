...
def mechanism_line(mechanism: dict[str, Any]) -> str:
    gate = mechanism["physical_gate"]
    tile = mechanism.get("output_tile_rows")
    tile_text = "" if tile is None else f" output-row-tile={tile};"
    return (
        f"- `{mechanism['mechanism']}`: G2={mechanism['G2_exact_transition']}, "
        f"G3={mechanism['G3_full_transformer_layer_boundary']}, G4={mechanism['G4_physical_saving']};"
        f"{tile_text} artifact={mechanism['artifact_bytes']} B; "
        f"reference-layer={mechanism['reference_layer_parameter_bytes']} B; "
        f"compiled-layer-resident={mechanism['compiled_layer_resident_parameter_bytes']} B; "
        f"peak-hot={mechanism.get('peak_projection_hot_bytes', 0)} B; "
...
def main() -> None:
    ...
    mechanism_text = "\n".join(mechanism_line(mechanism) for mechanism in mechanisms)
    dev, target = result["pins"]["DEV-W"], result["pins"]["TARGET-W"]
    dev_resolution, target_resolution = result["hf_resolution"]["DEV-W"], result["hf_resolution"]["TARGET-W"]
    common = f"""## Fixed-public dynamic executor hosted result — commit `{sha}` / run `{run_id}`

- Verdict: `{result['verdict']}`
- DEV-W: `{dev['repo_id']}@{dev['revision']}`; resolved SHA `{dev_resolution.get('resolved_sha')}`; official `LlamaForCausalLM.from_pretrained()` executed.
- TARGET-W: `{target['repo_id']}@{target['revision']}`; metadata access={target_resolution.get('accessible')}; full TARGET-W weights/performance remain unevaluated unless the gated tensors were actually accessible.
...
"""
    ...
    append_once(
        ...
        + f"- G2 EXACT TRANSITION: {any(mechanism['G2_exact_transition'] for mechanism in mechanisms)}.\n"
        + f"- G3 REAL BOUNDARY: {any(mechanism['G3_full_transformer_layer_boundary'] and mechanism['G2_exact_transition'] for mechanism in mechanisms)}.\n"
        + f"- G4 PHYSICAL SAVING: {any(mechanism['G4_physical_saving'] for mechanism in mechanisms)}.\n"
    ...
    append_once(
        ...
        "Use the committed actual-checkpoint tensor audit and physical ledger to modify only the measured failing resource term. "
        "If G4 failed for all implemented primitives, the next implementation must eliminate the observed artifact/residency/I/O/latency term rather than rename the same codec, tiling choice, or compiler primitive. "
        "If G4 passed, move directly to multi-layer accounting and an exact TARGET-W artifact/8GiB ledger when gated tensor access is available.\n\n"
        + common,
    )
    rejected = [mechanism for mechanism in mechanisms if not mechanism["G4_physical_saving"]]
    if rejected:
        append_once(
            ROOT / "FAILED_APPROACHES_RECENT.md",
            marker,
            "## Actual-checkpoint mechanism rejection evidence\n\n"
            + "\n".join(mechanism_line(mechanism) for mechanism in rejected)
            + "\n\n"
            + common,
        )
