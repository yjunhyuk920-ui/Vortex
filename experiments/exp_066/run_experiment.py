#!/usr/bin/env python3
"""Run EXP-066 pinned real-Q4 exact TT/MPO bond-rank Gate."""
...
def rank_one_control() -> tuple[np.ndarray, TensorTrainModePlan]:
    plan = TensorTrainModePlan(
        variant="rank_one_control",
        row_schedule=(2, 2, 2),
    ...
    return deinterleave_tensor(tensor, plan.mode_pairs), plan
...
def control_population(
    *,
    seed: int,
    primes: tuple[int, ...],
    maximum_modes: tuple[int, ...],
) -> tuple[list[dict[str, Any]], int, list[float]]:
    matrix, plan = rank_one_control()
    rank_one = certify_tt_plan(matrix, plan, primes=primes)
    mutated = matrix.copy()
    mutated[0, 0] += 1
    mutation = certify_tt_plan(mutated, plan, primes=primes)
    round_trip = deinterleave_tensor(
        interleave_matrix(matrix, plan.mode_pairs), plan.mode_pairs
    )
    ...
    definitions = (
        (
            "rank_one_mpo",
            rank_one.bond_rank_lower_bounds == (1, 1)
            and rank_one.witness_mismatches == 0,
            rank_one.as_dict(),
        ),
        (
            "one_nibble_mutation",
            mutation.maximum_bond_rank >= 2
            and mutation.witness_mismatches == 0,
            mutation.as_dict(),
        ),
        (
            "interleaved_round_trip",
            np.array_equal(round_trip, matrix),
    ...
    for index in range(4):
        ...
        passed = (
            all(item.witness_mismatches == 0 for item in plans)
            and max(item.maximum_bond_rank for item in plans) >= 4
        )
        failures += int(not passed)
        ...
        rows.append(
            {
                ...
                "maximum_certified_bond_rank": max(
                    item.maximum_bond_rank for item in plans
                ),
            ...
            rows.append(
                {
                    ...
                    "mode_pairs": [
                        list(pair) for pair in plan.mode_plan.mode_pairs
                    ],
                    "physical_dimensions": list(
                        plan.mode_plan.physical_dimensions
                    ),
                    "cut": cut,
...
def main() -> None:
    ...
    parser.add_argument(
        "--cache-dir",
        type=Path,
        default=ROOT / ".cache/exp_066_huggingface",
    )
        ...
        for tensor_name, tensor in sorted(state.items()):
            ...
            if plans:
                ...
                witness_mismatches += sum(
                    item.witness_mismatches for item in plans
                )
                ...
                selected_full_cuts = selected.full_unfolding_rank_cuts
                selected_max_bond = selected.maximum_bond_rank
            else:
                ...
                selected_modes = None
                selected_variant = "dense_fallback"
                selected_bonds = []
                selected_full_cuts = 0
                selected_max_bond = None
            row = {
                "model_id": model_id,
                "revision": revision,
                "tensor_name": tensor_name,
                ...
                "selected_maximum_bond_rank": selected_max_bond,
                "selected_full_unfolding_rank_cuts": selected_full_cuts,
    ...
    measured = {
        ...
        "dense_without_nontrivial_mode_plan": sum(
            row["mode_plan_count"] == 0 for row in dense_rows
        ),
        ...
        "selected_maximum_bond_rank_distribution": dict(
            Counter(
                str(row["selected_maximum_bond_rank"])
                for row in dense_rows
            )
        ),
        "selected_full_unfolding_rank_cut_count": sum(
            row["selected_full_unfolding_rank_cuts"] for row in dense_rows
        ),
    ...
    summary = {
        "experiment": "EXP-066",
        "name": "pinned_real_q4_exact_tt_mpo_bond_rank_gate",
        "phase": ["A", "B", "C-observation"],
        "evidence_level": "E1",
        "authoritative_decision": decision,
        "MEASURED": measured,
        "DERIVED": {
            "tt_mpo_lower_bound_survives_gate": lower_bound_survives,
            "decision": decision,
            **gates,
            "exact_integer_mpo_reconstruction_gate_pass": False,
            "accounting_scope": (
                "two-prime exact prefix/suffix unfolding-rank witnesses and "
                "favorable dense-core slot lower bounds for 4-bit MPO cores, "
