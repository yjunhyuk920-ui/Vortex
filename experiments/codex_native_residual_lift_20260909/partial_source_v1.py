...
def run_case(name, B, v, alpha0, alpha1):
    ...
    alpha0_q = bf16_fraction(alpha0)
    k0_unclamped = [round_even(bf16_fraction(y) / alpha0_q) for y in y0]
    ...
    alpha1_q = bf16_fraction(alpha1)
    correction = [round_even(bf16_fraction(y) / alpha1_q) for y in y1]
    decoded = [k0[i] + correction[i] for i in range(m)]
    parity = [value % 2 for value in decoded]
    truth = [int(row @ v) for row in B]
    ...
    return {
        ...
        "numeric_accounting": {
            ...
            "compiler_total_coefficient_writes": int(m * D), "two_matvec_matrix_reads": int(2 * m * D),
            "two_matvec_products": int(2 * m * D), "two_matvec_padded_zero_leaves": int(2 * m * (P - D)),
            "two_matvec_reduction_additions": int(2 * m * (P - 1)),
            "two_input_vector_lanes_written": int(2 * D), "query2_bias_lanes_set": int(sum(x.bit_count() for x in k0)),
            "decode_exact_rational_divisions": int(2 * m), "decode_round_even_calls": int(2 * m),
        },
    }
...
def main():
    ...
    result = {
        ...
        "platform": platform.platform(), "env_UTF8": os.environ.get("UTF8"),
