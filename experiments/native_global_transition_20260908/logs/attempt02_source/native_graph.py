...
class NativeGraphExecutor:
    ...
    def forward(self,*args,**kwargs):
        ...
        # Explicit value guard for the observed native SDPA skip branch. This
        # is not permission to ignore arbitrary padding or data-dependent code.
        ids=kwargs.get('input_ids')
        if ids is None or ids.ndim!=2 or ids.shape[0]!=1:raise ValueError('Batch-one token-ID ABI')
        past=cache.get_seq_length() if cache is not None else 0
        if past and ids.shape[1]!=1:raise ValueError('Only empty-cache prefill or single-token decode')
        if self.model.config._attn_implementation!='sdpa':raise ValueError('Pinned SDPA only')
        mask=kwargs.get('attention_mask')
        mask_guard_bytes=0
        if mask is not None:
            if not isinstance(mask,torch.Tensor) or mask.ndim!=2 or mask.shape!=(1,past+ids.shape[1]):
                raise ValueError('Unpadded rectangular mask required')
            mask_guard_bytes=mask.numel()*mask.element_size()
            if not bool(torch.all(mask==1)):raise ValueError('Padding requires a separate captured branch')
            kwargs=dict(kwargs);kwargs['attention_mask']=None
        ...
        sig={'names':names,'meta':[tensor_meta(t) for t in inputs],'const':encode(const),
             'past_layers':len(flat)//2,'mask_guard_input_bytes':mask_guard_bytes}
        ...
        outputs=self.execute(self.programs[key],inputs)
...
class DemandGraphExecutor(NativeGraphExecutor):
    """A finite {TOP, singleton tensor} native-domain demand worklist.

    Output domains start TOP: no desired logits/future KV supplied. Inverse
    projection of TOP through a total guarded native function is TOP, hence no
    backward narrowing occurs. Required functional nodes are then evaluated
    exactly once in native dependency order, with actual costs retained.
    """
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs);self.demand_records=[]

    def execute(self,program,inputs):
        nodes=list(program.graph.nodes);output=next(n for n in nodes if n.op=='output')
        demanded=set();todo=[output];visits=0
        while todo:
            n=todo.pop();visits+=1
            if n in demanded:continue
            demanded.add(n);todo.extend(n.all_input_nodes)
        env={};it=iter(inputs);users=collections.Counter()
        for n in demanded:
            for p in n.all_input_nodes:users[p]+=1
        calls=0;matrix_calls=0;hist=collections.Counter()
        for n in nodes:
            if n.op=='placeholder':env[n]=next(it);continue
            if n not in demanded:continue
            args=torch.fx.map_arg(n.args,lambda x:env[x]);kwargs=torch.fx.map_arg(n.kwargs,lambda x:env[x])
            if n.op=='get_attr':value=resolve_attr(program,n.target)
            elif n.op=='call_function':
                value=n.target(*args,**kwargs);calls+=1;hist[str(n.target)]+=1
                if str(n.target) in ('aten.mm.default','aten.bmm.default','aten.addmm.default'):matrix_calls+=1
            elif n.op=='output':
                self.demand_records.append({'domain_kind':'TOP or exact native singleton',
                    'supplied_desired_output_bits':0,'backward_narrowings':0,
                    'demand_stack_visits':visits,'native_calls':calls,'matrix_calls':matrix_calls,
                    'operator_histogram':dict(sorted(hist.items())),
                    'state_or_output_oracle':False})
                return args[0]
            else:raise ValueError(f'Unknown execution node {n.op}')
            env[n]=value
            for p in n.all_input_nodes:
                users[p]-=1
                if users[p]==0 and p in env:del env[p]
