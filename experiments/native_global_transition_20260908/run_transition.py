...
from native_graph import NativeGraphExecutor,DemandGraphExecutor,raw_bytes,cache_flat,tensor_meta
...
def arm_run(model,original,executor,root,arm,prefix,seed,explicit_mask=False):
    ...
    try:
        ...
        with torch.inference_mode():
            generation=dict(input_ids=torch.tensor([prefix],dtype=torch.long),
            ...
            if explicit_mask:
                generation['attention_mask']=torch.ones((1,len(prefix)),dtype=torch.long)
        ...
        report={'arm':arm,'seed':seed,'prefix':prefix,'steps':steps,
                ...
                'explicit_all_one_mask':explicit_mask,
                'explicit_pad_matches_previous_HF_auto_pad':explicit_mask,
...
def run(outdir,explicit_mask=False):
    ...
    demand=DemandGraphExecutor(model,original,outdir/'demand_programs')
    ...
    for prefix,seed in zip(PREFIXES,SEEDS):
        for arm,ex in [('reference',None),('graph',graph),('packed_state',storage),('demand',demand)]:
            rr=arm_run(model,original,ex,outdir,arm,prefix,seed,explicit_mask);runs.append(rr)
            print(arm,seed,rr['sequence'],len(rr['steps']),flush=True)
        ref,g,c,d=runs[-4:]
        comparisons.append({'seed':seed,'graph_errors':compare_runs(ref,g),
                            'packed_state_errors':compare_runs(ref,c),'demand_errors':compare_runs(ref,d)})
    ...
    result={'source_pin':'HuggingFaceTB/SmolLM2-135M@93efa2f097d58c2a74874c7e644dbc9b0cee75a2',
            ...
            'graph_programs':graph_summary,'storage_records':storage.records,'demand_records':demand.demand_records,
            'demand_constructor_original_forward_calls':len(demand.records),
            'demand_constructor_seconds':sum(r['build_seconds'] for r in demand.records),
    ...
    assert not prefill_errors and all(not r['graph_errors'] and not r['packed_state_errors'] and not r['demand_errors'] for r in comparisons)
    assert result['weights_unchanged']
    print(json.dumps({k:v for k,v in result.items() if k not in ('graph_programs','storage_records','demand_records')},ensure_ascii=False),flush=True)


if __name__=='__main__':
    p=argparse.ArgumentParser();p.add_argument('--out',default=str(ROOT/'results/run'))
    p.add_argument('--explicit-mask',action='store_true')
    args=p.parse_args();run(args.out,args.explicit_mask)
