...
def main():
    ...
    with torch.inference_mode():
        ...
        for step,token in enumerate(tokens):
            ...
            equality=(out.logits.dtype==logits.dtype and raw(out.logits)==raw(logits) and native_words==candidate_words)
            records.append({'step':step,'token':token,'exact':equality,'native_logits_dtype':str(out.logits.dtype),'counts':stats})
            if not equality:
                first_failure={'step':step,'token':token,'native_logits':raw(out.logits).hex(),'compiled_logits':raw(logits).hex(),
                    'native_cache':native_words,'compiled_cache':candidate_words}
                break
    ...
    if first_failure is None:
        ...
        sampled=list(prefix)
        cache=None
        with torch.inference_mode(),prohibit_dense_forward(model):
            for token in prefix:
    ...
    if first_failure is not None or not full_incremental_equal or not deterministic_rng_unchanged or (generation and not all(generation[k] for k in ['tokens_exact','RNG_consumption_and_next_state_exact','final_cache_exact'])):
