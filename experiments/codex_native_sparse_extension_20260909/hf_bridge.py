...
def cache_state(value):
    ...
    if isinstance(value,(list,tuple)):
        return {'container':type(value).__name__,'items':[cache_state(item) for item in value]}
    if isinstance(value,dict):
        return {key:cache_state(item) for key,item in sorted(value.items())}
    if type(value).__module__=='transformers.cache_utils':
        return {'class':type(value).__module__+'.'+type(value).__qualname__,'fields':cache_state(vars(value))}
    raise TypeError(f'unaccounted cache state type: {type(value)}')


def cache_alias_graph(cache):
    tensors=[tensor for layer in cache.layers for tensor in (layer.keys,layer.values)]
    return [[a.untyped_storage().data_ptr()==b.untyped_storage().data_ptr() for b in tensors] for a in tensors]
    ...
    with torch.inference_mode():
        ...
        for layer in model.model.layers:
            ...
            for j in range(n):
                ...
                if raw(predicted.view(torch.bfloat16))!=raw(normalized[j]):
                    raise ValueError('native RMSNorm words are outside the compiled family')
...
def compiled_step(compiled,token,cache=None):
    ...
    for i in range(compiled.layers):
        ...
        # Match native view(batch,time,heads,head_dim).transpose(1,2),
        # including its singleton-time stride on the first cache update.
        v=vector.reshape(1,1,compiled.m//compiled.hd,compiled.hd).transpose(1,2)
...
def main():
    ...
    manifest={'config':model.config.to_dict(),'torch':torch.__version__,
        ...
        'python_optimize':sys.flags.optimize,'platform':platform.platform(),'processor':platform.processor(),
        'cpu_capability':torch.backends.cpu.get_cpu_capability(),
        'invocation':[Path(sys.argv[0]).name]+sys.argv[1:],
        'relevant_environment':{name:os.environ.get(name) for name in ['OMP_NUM_THREADS','MKL_NUM_THREADS','DNNL_MAX_CPU_ISA','ATEN_CPU_CAPABILITY','PYTHONOPTIMIZE']},
        'executed_source_sha256':{name:hashlib.sha256((Path(__file__).parent/name).read_bytes()).hexdigest() for name in ['hf_bridge.py','scalar_producer.py','PREREGISTRATION.md']}}
    ...
    with torch.inference_mode():
        ...
        full_logits_exact=full.logits.dtype==torch.bfloat16 and raw(full.logits)==raw(torch.zeros((1,len(tokens),compiled.n),dtype=torch.bfloat16))
        ...
        for step,token in enumerate(tokens):
            ...
            state_equal=(cache_state(native_cache)==cache_state(compiled_cache) and cache_alias_graph(native_cache)==cache_alias_graph(compiled_cache))
            ...
            records.append({'step':step,'token':token,'exact':equality,'cache_serialized_fields_exact':state_equal,
                'cache_internal_alias_graph_exact':cache_alias_graph(native_cache)==cache_alias_graph(compiled_cache),
    ...
    if first_failure is None:
        ...
        with torch.inference_mode(),prohibit_dense_forward(model),sampling_trace(compiled_samples):
            ...
            for step in range(8):
                ...
                compiled_generation_logits.append(raw(scores).hex())
        ...
        generation={'native_tokens':reference.sequences[0].tolist(),'compiled_tokens':sampled,
            ...
            'final_cache_serialized_fields_and_internal_aliases_exact':native_sample_state==cache_state(cache) and cache_alias_graph(reference.past_key_values)==cache_alias_graph(cache),
            'each_sample_logits_exact':len(reference.logits)==8 and [raw(item).hex() for item in reference.logits]==compiled_generation_logits,
            'each_sample_probability_RNG_exact':len(native_samples)==8 and native_samples==compiled_samples,
    ...
    result={'kind':'E1_RESTRICTED_TWO_SUPPORT_BF16_CAUSAL_PRODUCER','torch':torch.__version__,
        ...
        'cache_serialized_fields_and_internal_aliases_exact':cache_metadata_exact,
        'native_full_prefill_incremental_KV_words_exact':full_incremental_equal,
        'native_full_prefill_logits_exact':full_logits_exact,
    ...
    print(json.dumps({k:result[k] for k in ['manual_steps','manual_all_logits_KV_exact','native_full_prefill_incremental_KV_words_exact','deterministic_native_and_compiled_RNG_unchanged','generation','checkpoint_unchanged']}))
    if first_failure is not None or not full_logits_exact or not full_incremental_equal or not deterministic_rng_unchanged or (generation and not all(generation[k] for k in ['tokens_exact','RNG_consumption_and_next_state_exact','final_cache_exact','final_cache_serialized_fields_and_internal_aliases_exact','each_sample_logits_exact','each_sample_probability_RNG_exact'])):
