...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Profile exact-vs-capsule output error by row tile, then repair "
            "the highest-value tiles and measure the optimistic exact-weight "
            "budget needed to restore the greedy sequence."
        )
    )
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("oracle_tile_repair.json"),
    )
...
def configure_top_tiles(
    ...
    selected: dict[str, list[int]] = {name: [] for name in replacements}
    for item in candidates[:count]:
        selected[str(item["module"])].append(int(item["tile_index"]))
    for name, module in replacements.items():
        module.configure_row_tile_repair(
            tile_rows=tile_rows,
            tile_indices=selected[name],
        )
        module.set_mode("project_repair")
    return sum(module.selected_repair_bytes for module in replacements.values())
...
def main() -> None:
    args = parse_args()
    if args.tile_rows <= 0:
        raise SystemExit("--tile-rows must be positive")
    ...
    for module in replacements.values():
        module.reset_tile_profile(tile_rows=args.tile_rows)
    ...
    candidates: list[dict[str, Any]] = []
    for name, module in replacements.items():
        for tile in module.profiled_row_tiles():
            candidates.append({"module": name, **tile})
    candidates.sort(
        key=lambda item: (
            float(item["error_per_byte"]),
            float(item["error_sum"]),
        ),
        reverse=True,
    )
    ...
    rejection_budget_match: dict[str, int | float | bool | None] | None = None

    if not all_project_match:
        for count, candidate in enumerate(candidates, start=1):
            cumulative_bytes += int(candidate["weight_bytes"])
            if cumulative_bytes > max_rejection_bytes:
                break
            ...
            tested.append(item)
            if item["exact_sequence_match"]:
                rejection_budget_match = item
                if exact_bytes <= max_gate_bytes:
    ...
    existing_counts = {int(item["selected_tiles"]) for item in tested}
    first_coarse_match = rejection_budget_match
    if first_coarse_match is None and not all_project_match:
        for count in coarse_counts:
            if count <= 0 or count > len(candidates) or count in existing_counts:
                continue
            exact_bytes = configure_top_tiles(
                replacements,
                candidates,
            ...
            tested.append(item)
            if item["exact_sequence_match"]:
                first_coarse_match = item
                break

    elapsed = time.perf_counter() - started
    if all_project_match:
        decision = "all-project capsule matched this sequence; broader prompts required"
    elif gate_budget_match is not None:
        decision = "row-tile oracle meets Gate 0 byte envelope on this sequence"
    elif rejection_budget_match is not None:
        decision = "row-tile oracle restores output only below Gate 0 efficiency"
    else:
        decision = "top-error row-tile oracle fails within E>=300 rejection budget"

    result = {
        "evidence_level": "E1 optimistic row-tile oracle",
        "model": args.model,
        "device": str(device),
        "dtype": str(dtype),
        ...
        "first_repair_match": first_coarse_match,
        "tested_prefixes": sorted(
            tested,
            key=lambda item: int(item["selected_tiles"]),
        ),
        "top_profiled_tiles": candidates[:32],
        "decision": decision,
        "elapsed_seconds": elapsed,
        "scope_note": (
            "Optimistic oracle: exact evaluation errors rank tiles, prefill "
            "repair is not charged separately, and the top-prefix search is "
            "not guaranteed to find the globally optimal tile subset."
        ),
    }
