...
def profile_exact_target_margin_tiles(
    *,
    ...
    """Profile first-order exact-target margin influence for all residual tiles.

    Gradients are requested directly with ``torch.autograd.grad`` from the
    scalar margin objective to every captured projection output. This avoids
    depending on non-leaf ``Tensor.grad`` retention and makes disconnected
    outputs explicit through ``allow_unused=True``.
    """
    ...
    try:
        ...
        if exact_sequence[: len(prompt_ids)] != prompt_ids:
            raise RuntimeError("exact sequence does not preserve prompt prefix")
