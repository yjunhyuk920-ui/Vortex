...
zero at structurally eligible positions may skip Value accumulation. Causal,
padding, and registered local-window mask zeros are excluded from savings. QK
scoring and softmax remain fully charged because zero status is known only after
...
class AttentionProbabilityAccounting:
    ...
    structural_masked_probability_count: int
    ...
    def as_dict(self) -> dict[str, Any]:
        return {
            ...
            "structural_masked_probability_count": self.structural_masked_probability_count,
...
def structural_eligible_mask(
    *,
    ...
    """Return causal and optional local-window eligibility for unpadded batches."""
    ...
    if local_window_size is not None and local_window_size <= 0:
        raise AttentionProbabilitySparsityError(
            "local attention window must be positive"
        )
    query_positions = past_length + torch.arange(query_length, device=device)
    key_positions = torch.arange(key_length, device=device)
    eligible = key_positions.unsqueeze(0) <= query_positions.unsqueeze(1)
    if local_window_size is not None:
        lower = query_positions.unsqueeze(1) - int(local_window_size) + 1
        eligible = eligible & (key_positions.unsqueeze(0) >= lower)
    return eligible.reshape(1, 1, query_length, key_length).expand(
        batch_size, head_count, query_length, key_length
    )


def causal_eligible_mask(**kwargs: Any) -> Any:
    """Backward-compatible global-attention eligibility wrapper."""

    return structural_eligible_mask(**kwargs)
...
def account_attention_probabilities(
    ...
    *,
    ...
    if attention_kind not in {"global", "local"}:
        raise AttentionProbabilitySparsityError(
            f"unsupported attention kind: {attention_kind}"
        )
    if attention_kind == "global" and local_window_size is not None:
        raise AttentionProbabilitySparsityError(
            "global attention cannot declare a local window"
        )
    if attention_kind == "local" and local_window_size is None:
        raise AttentionProbabilitySparsityError(
            "local attention requires a window size"
        )
    if head_dimension <= 0:
    ...
    eligible = structural_eligible_mask(
        torch=torch,
    ...
    if masked_values.numel() and bool((masked_values != 0).any()):
        raise AttentionProbabilitySparsityError(
            "structurally masked attention entries are not exactly zero"
        )
    ...
    structurally_masked = int(probabilities.numel()) - eligible_count
    ...
    return AttentionProbabilityAccounting(
        ...
        structural_masked_probability_count=structurally_masked,
