...
def compile_checkpoint_inductor(reference_model: nn.Module, *, artifact_dir: Optional[str | Path] = None, layer_index: int = 0) -> CompiledArtifact:
    ...
    root = Path(artifact_dir) if artifact_dir is not None else Path(tempfile.mkdtemp(prefix="fixed-public-inductor-")); root.mkdir(parents=True, exist_ok=True)
    os.environ["TORCHINDUCTOR_CACHE_DIR"] = str(root / "torchinductor")
